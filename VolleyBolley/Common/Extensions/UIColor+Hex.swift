...
extension UIColor {
    ...
    ///
    /// - Параметры:
    ///   - hex: Строка с шестнадцатеричным кодом цвета. Может начинаться с символа `#` или без него.
    ///   Должна содержать 6 символов (например, `"#FF5733"` или `"FF5733"`).
    ///   - alpha: Необязательное значение прозрачности от 0.0 до 1.0. По умолчанию — 1.0 (полностью непрозрачный).
    ///
    /// - Примечание: Поддерживаются только шестизначные значения (формат RGB).
    /// Короткий формат (`#FFF`) и значения с альфой (`#RRGGBBAA`) не поддерживаются.
    ///
